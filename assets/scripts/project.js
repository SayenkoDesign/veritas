...
(function (document, window, $) {
	...
	// clone header
	var $fixed_header_cloned = $site_header.clone().prop('id', 'masthead-fixed').attr('aria-hidden', 'true').addClass('fixed').insertBefore('#masthead');
...
(function (document, $, undefined) {
	...
	child_theme.init = function () {
		...
		$('.site-header:not(.fixed)').find('.' + nav_menu).after(toggleButtons.menu); // add the main nav buttons
	...
	function _addClassID() {
		var $this = $(this),
		    nav = $this.prev('nav'),
		    id = 'class';
		$this.addClass('mobile-' + $(nav).attr('class'));
		if ($(nav).attr('id')) {
			id = 'id';
		}
